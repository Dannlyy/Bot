...
class File {
    ...
    public const ENUMERATION = File::ENUM;
    ...
    public static $formats = [
        ...
        "yaml" => File::YAML,
        "sl" => File::SERIALIZED,
        "serialize" => File::SERIALIZED,
        "txt" => File::ENUM,
        "list" => File::ENUM,
        "enum" => File::ENUM
    ];

    public function __construct(string $file, int $type = File::DETECT, array $default = [], &$correct = null){
    ...
    public function load(string $file, int $type = File::DETECT, array $default = []) : bool{
        $this->correct = true;
        $this->file = $file;

        $this->type = $type;
        if($this->type === File::DETECT){
            $extension = explode(".", basename($this->file));
            $extension = strtolower(trim(array_pop($extension)));
            if(isset(File::$formats[$extension])){
                $this->type = File::$formats[$extension];
            }else{
                $this->correct = false;
            }
        }
    ...
    public function setJsonOptions(int $options) : File {
        if($this->type !== File::JSON){
    ...
    public function enableJsonOption(int $option) : File {
        if($this->type !== File::JSON){
    ...
    public function disableJsonOption(int $option) : File {
        if($this->type !== File::JSON){
    ...
    private function parseProperties(string $content) : array{
        ...
        if(preg_match_all('/^\s*([a-zA-Z0-9\-_\.]+)[ \t]*=([^\r\n]*)/um', $content, $matches) > 0){ //false or 0 matches
            foreach($matches[1] as $i => $k){
                ...
                if(isset($result[$k])){
                    echo("[Config] Repeated property " . $k . " on file " . $this->file);
                }
                $result[$k] = $v;
            }
        }
